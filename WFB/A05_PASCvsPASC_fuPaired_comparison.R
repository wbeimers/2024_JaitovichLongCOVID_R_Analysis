...
# options:
# analysis_group (1, 2, 3, 4, 5)
anal <- 2
# comparison (Age, Sex, QoL, BMI)
comp <- "PASC_vs_PASC_fu"
# formula (1, 2, 3, etc.)
form <- 34
# ome
omea <- "transcript"
...
ggsave(paste0("reports/figures/Volcano_group_", anal, "_", comp, "_formula", form, "_pairedTtest.pdf"), 
...
#### PASC+PASCfu vs QoL LRM ####

# Limit df to PASC/PASC_fu from pre-tube change
filtered_df_a_2 <- filtered_df_a %>%
...
ggplot(filtered_df_a_2 %>% filter(biomolecule_id == bm), 
       aes(x = normalized_abundance, y = SF.36.QOL.Score)) +
  geom_point(size = 1) +
  geom_smooth(method = "lm") +
  labs(title = "Change in QoL vs Change in Biomolecule")
ggsave(paste0('reports/figures/SingleProteinPlots/AnalysisGroup2_PASCvsPASC_fu_singlebm_', bm, '_scatter_Cohort.pdf'), 
...
asdf <- patient_metadata %>%
  filter(analysis_group_4 == 1)
